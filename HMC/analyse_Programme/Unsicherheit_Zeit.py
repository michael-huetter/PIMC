...
start_step = 500
skip = 100
Zeit = 17 #gesammtzteit für eine Temeratur (gerade 85s bei HMC)
Zeit_step = Zeit/N
...
secax.set_xlabel('Zeit (s)')
ax.set_xlabel('Steps')
ax.set_ylabel("Absulute Position")
plt.show()

# Alle drei Plots nebeneinander in einem Bild
fig, axes = plt.subplots(1, 2, figsize=(10, 5), constrained_layout=True)

daten = [
    (Ekin_array, "Ekin-Mittelwert über Schritte"),
    (Epot_array, "Epot-Mittelwert über Schritte"),
]

for ax, (array, title) in zip(axes, daten):
    ax.plot(steps, array[0], "or")
    ax.errorbar(steps, array[0], yerr=array[1])
    ax.grid(linestyle="--")
    ax.set_title(title)
    ax.set_xlabel("Steps")
    ax.set_ylabel("Energie")
