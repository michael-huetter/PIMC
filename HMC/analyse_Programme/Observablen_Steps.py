...
pot_arc = af.autokorr(Epot[Skip:],400)
kin_arc = af.autokorr(Ekin[Skip:],400)
...
axs[0].plot(Steps[Skip:], Ekin[Skip:])
axs[0].plot([0, N], [Ekin_mean, Ekin_mean])
axs[0].set_title(f"Kinetische Energie ({T=})")
axs[0].legend(["Samplepoints", f"Mean = {Ekin_mean:.5f} $\\pm$ {delta_Ekin:.5f}"])
axs[0].set_xlabel("Steps")
axs[0].set_ylabel("Energy")

# Plot 2: Potenzielle Energie
axs[1].plot(Steps[Skip:], Epot[Skip:])
