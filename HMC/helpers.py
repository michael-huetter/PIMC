...
def save_to_csv(data, filename):
    """
    Save resutls to /output_1/ with a unique filename if multiple T loops are used
    """

    unique_filename = _get_unique_filename(f"output_{HMC}/" + str(filename))
...
def wOut(message, filename=f'output_{HMC}.out'):
