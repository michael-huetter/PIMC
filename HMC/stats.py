...
def rcc():
    ...
    if len(T) == 0:
        wOut(f"Error: No data found in output_1 directory, make sure the potential is properly defined and appropriat input parameters are used. \
Alternatively, try the logging mode.")
        exit(f"Error: No data found in output_1 directory, make sure the potential is properly defined and appropriat input parameters are used. \
Alternatively, try the logging mode.")
    ...
    E_trace = []
    for i in T:
        kin = np.loadtxt("output_1/" + i + "_KinEnergyTrace.csv")
        pot = np.loadtxt("output_1/" + i + "_PotEnergyTrace.csv")
        e = np.mean(kin + pot)
    ...
    for i in T:
        ...
        data = np.loadtxt(f"output_1/{i}_eStatTrace.csv", delimiter=",")
        _eStatePop(data, i)
