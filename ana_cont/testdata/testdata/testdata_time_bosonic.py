...
norm=np.trapezoid(aw[1:]/wgrid[1:],wgrid[1:])
aw=aw/norm
...
for i,tau in enumerate(taugrid):
    ...
    integrand[0]=0.
    chi_tau[i]=np.trapezoid(integrand,wgrid)

chi_iw=np.zeros_like(iwgrid, dtype=complex)
for i,iw in enumerate(iwgrid):
    chi_iw[i]=np.trapezoid(chi_tau*np.exp(1j*taugrid*iw),taugrid)

chi_iw_direct=np.zeros_like(chi_iw)
for i,iw in enumerate(iwgrid):
    chi_iw_direct[i]=np.trapezoid(aw*wgrid/(wgrid**2+iw**2),wgrid)
chi_iw_direct[0]=np.trapezoid(aw[1:]/wgrid[1:],wgrid[1:])
