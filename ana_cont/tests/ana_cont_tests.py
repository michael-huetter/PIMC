...
class TestMaxentFermionic(unittest.TestCase):
    def setUp(self):
        ...
        giw = np.trapezoid(kernel * spec[None, :], w, axis=1)
...
class TestMaxentBosonic(unittest.TestCase):
    def setUp(self):
        ...
        spec_real /= np.trapezoid(spec_real, w_real)  # normalization
        ...
        kernel[0, 0] = 1.
        gf_bos = np.trapezoid(kernel * spec_real[None, :], w_real, axis=1) + noise
        norm = gf_bos[0]
        gf_bos /= norm
