...
class MaxentSolverSVD(AnalyticContinuationSolver):
    ...
    def backtransform(self, A):
        ...
        """
        return np.trapezoid(np.dot(self.ucov, self.kernel.matrix) * A[None, :],
                        self.re_axis, axis=-1)
    ...
    def entropy_posneg(self, A, u):
        ...
        return np.trapezoid(root - self.model_plus - self.model_minus
    ...
    def solve_bryan(self, alphastart=500, alphadiv=1.1, interactive=False):
        ...
        probarr /= -np.trapezoid(probarr, alpharr)  # normalize the probability distribution
        if interactive:
            fig = plt.figure()
        ...
        A_opt = -np.trapezoid(specarr * probarr[:, None], alpharr,
